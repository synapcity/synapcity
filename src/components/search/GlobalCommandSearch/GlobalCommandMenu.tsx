      ...
      <CommandItem onSelect={() => onNavigate(`/home/notes/${note.id}`)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold truncate">
            {highlightMatches(note.title, matches?.filter((m) => m.key === "title"))}
          </div>
          {note.summary && (
            <div className="text-xs text-gray-400 truncate">
              {highlightMatches(note.summary, matches?.filter((m) => m.key === "summary"))}
            </div>
          )}
        </div>
        <button
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold truncate">
                ...
                <span className="italic">
                  {highlightMatches(view.viewLabel, matches?.filter((m) => m.key === "viewLabel"))}
                </span>
              </div>
              {view.content && (
                <div className="text-xs text-gray-500 truncate">
                  {highlightMatches(view.content, matches?.filter((m) => m.key === "content"))}
                </div>
              )}
            </div>
          </CommandSubItem>
...
export function GlobalCommandMenu() {
  const {
    open, setOpen, rawQuery, setRawQuery, clearQuery,
    page, setPage, recents, addRecent, pins, pin, unpin, isPinned
  } = useCommandMenuStore();
  ...
  const viewsArr = useMemo(() => Object.values(viewMap).filter((v) => v.type === "editor"), [viewMap]) ?? [];
  ...
  const handleNavigate = useCallback((url: string, id: string) => {
  ...
  return (
    <CommandDialog
      ...
      title="Global Search"
      description={rawQuery.trim() === "" ? "Jump to notes, views, and more…" : "Search your workspace"}
      showCloseButton
      shouldFilter={false}
    >
      <CommandInput
            ...
            {pinsItems.map(item => (
              ...
              >
                ...
                <button
                  ...
                  tabIndex={-1}
                  aria-label="Unpin"
                  onClick={e => { e.stopPropagation(); unpin(item.id); }}>
