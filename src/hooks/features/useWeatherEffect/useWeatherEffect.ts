...
export function useWeatherEffect() {
  ...
  const setJustFetchedManually = useWeatherStore((s) => s.setJustFetchedManually);
  ...
  const fetchAndUpdate = useCallback(async () => {
    ...
    if (result) {
      ...
      if (alertsEnabled && result.rainSoon) {
        toast("Rain Alert", {
          description: "☔️ Rain is likely within the next hour near your location.",
        });
      }
    }
  }, [justFetchedManually, preferences, setLoading, unit, setJustFetchedManually, setData]);
