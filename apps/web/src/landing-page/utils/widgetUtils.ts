...
import { type AvailableWidget, type Widget, type WidgetType } from "@/landing-page/types";
...
export const createWidget = (
  ...
  if (!result.success) {
    console.warn(`Invalid widget props for ${availableWidget.type}`, result.error);
    return null;
  }

  const { minWidth, minHeight, maxWidth, width, height, maxHeight } = mergedProps;
