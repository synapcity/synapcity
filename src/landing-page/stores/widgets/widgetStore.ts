...
export const useWidgetStore = create<WidgetStore>((set, get) => ({
  ...
  loadMoreWidgets: (count = 10) => {
    ...
    set((state) => {
      ...
      const createdWidgets = newWidgets.map((widget) => createWidget(widget)).filter(Boolean);
  ...
  loadAvailableWidgets: (widgets) =>
    set((state) => ({
      availableWidgets: [
        ...
        ...widgets.filter((w) => !state.availableWidgets.some((aw) => aw.type === w.type)),
      ],
    })),
  addWidget: (widget, initialProps = {}) => {
    const available = get().availableWidgets.find((widget) => widget === widget);
    if (!available) return;
  ...
  getWidgetByType: (type) => get().availableWidgets.find((w) => w.type === type),
}));
