...
describe("themeStore", () => {
  ...
  it("can init and set scoped preferences", () => {
    ...
    expect(testStore.getState().scopedPreferences.note["abc"].fontSize).toBe("xl");
  });

  it("can get scoped preferences that inherit", () => {
    testStore.getState().initScopedPreferences("note", "abc");
    testStore.getState().setPreferences("note", "abc", { inheritsFromGlobalTheme: true });
  ...
  it("can reset scoped preferences", () => {
    ...
    testStore.getState().setPreferences("dashboard", "dash-1", { fontSize: "sm" });
    testStore.getState().resetScopedPreferences("dashboard", "dash-1");
    expect(testStore.getState().scopedPreferences.dashboard["dash-1"]).toBeUndefined();
  });
  ...
  it("can toggle scoped mode independently", () => {
    ...
    testStore.getState().setPreferences("widget", "widget-1", { mode: "light" });
    testStore.getState().toggleScopedMode("widget", "widget-1");
    expect(testStore.getState().scopedPreferences.widget["widget-1"].mode).toBe("dark");
  });
  ...
  it("toggles scoped mode from dark to light", () => {
    ...
    expect(testStore.getState().scopedPreferences.note["toggle-test"].mode).toBe("light");
  });
  ...
  it("removes inheritsFromGlobalTheme when previously true and not explicitly updated", () => {
    ...
    expect(
      testStore.getState().scopedPreferences.note["remove-inherit"].inheritsFromGlobalTheme
    ).toBe(false);
  });
  ...
  it("does not override inheritsFromGlobalTheme when already false", () => {
    ...
    expect(testStore.getState().scopedPreferences.note["keep-false"].inheritsFromGlobalTheme).toBe(
      false
    );
  });
