...
export const richPlugins: Array<() => Promise<{ default: any }>> = [
	...
	() =>
		import("../plugins/UIPlugins/SlashCommandMenu/SlashCommandMenuPlugin").then(
			(m) => ({
...
export const widgetPlugins: Array<() => Promise<{ default: any }>> = [
	() =>
		import("../plugins/BehaviorPlugins/Search/SearchHighlightPlugin").then(
			(m) => ({
	...
	() =>
		import(
			"../plugins/BehaviorPlugins/Draggable/CustomDraggableBlockPlugin"
		).then((m) => ({
			default: m.default,
		})),
	() =>
	...
	() =>
		import("../plugins/BehaviorPlugins/KeyboardMove/KeyboardMove").then(
			(m) => ({
	...
	() =>
		import("../plugins/UIPlugins/FloatingToolbar/FloatingToolbarPlugin").then(
			(m) => ({
