...
import { generateSemanticColor, applyColorVars, DEFAULT, applyThemeVars } from "@/theme";
import type { FontFamilyName, FontSizeToken, ThemePreferences, ThemeMode } from "@/theme/types";
...
export function useThemeEngine({
  ...
  const applyThemeStyles = useCallback(
    (prefs: Partial<ThemePreferences>, modeOverride?: ThemeMode) => {
      const element = (isGlobal ? document.body : targetRef.current) as HTMLElement;
