...
export default function TableCell({
  ...
  if (column.id === "linkedResources") {
    return (
      <td>
        <LinkedResourcesCell resources={cell.getValue()} />
      </td>
    );
  }
  if (column.id === "actions") {
    return (
      <td>
        <RowActionsCell row={row.original} onDelete={onDelete} onUpdate={onUpdate} />
      </td>
    );
  }

  return <UITableCell>{flexRender(cell.column.columnDef.cell, cell.getContext())}</UITableCell>;
}
