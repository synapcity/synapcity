...
const lightScale = Object.fromEntries(keys.map((k) => [k, `#${k}`])) as Record<ColorShade, string>;

const darkScale = Object.fromEntries(keys.map((k) => [k, `#dark-${k}`])) as Record<
  ColorShade,
  string
>;
...
describe("generateColorVars", () => {
  it("returns correct vars for light mode", () => {
    ...
    expect(result["--primary-foreground"]).toBe(mockColor.light.scale[950]);
    expect(result["--primary-background"]).toBe(mockColor.light.scale[50]);
  });
