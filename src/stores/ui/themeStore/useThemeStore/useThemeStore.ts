...
import { resolveThemeMetadata, type ThemeMetadataInfo } from "@/theme/utils/resolveThemeMetadata";
...
export interface ScopedThemeState {
  ...
  setPreferences: (scope: EntityType, id: string, updates: Partial<ThemePreferences>) => void;
...
export const themeStoreInitializer: StateCreator<ThemeStore> = (set, get, store) => ({
  ...
  setPreferences: (scope, id, updates) => {
    set((state) => {
      const prev =
        state.scopedPreferences[scope]?.[id] ?? getDefaultTheme(state.globalPreferences.mode);
  ...
  toggleGlobalMode: () => {
    set((state) => {
      const newMode = state.globalPreferences.mode === "dark" ? "light" : "dark";
      return {
        globalPreferences: {
  ...
  toggleScopedMode: (scope, id) => {
    set((state) => {
      const currentMode = state.scopedPreferences[scope][id].mode ?? state.globalPreferences.mode;
