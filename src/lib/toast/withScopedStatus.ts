...
export async function withToast<T>(
  ...
  try {
    if (messages.loading) showToast.info(messages.loading, { entityId: messages.entityId });
    const result = await action();
    showToast.success(
      typeof messages.success === "function" ? messages.success(0) : messages.success,
      {
    ...
    showToast.error(
      typeof messages.error === "function" ? messages.error(0) : messages.error || fallback,
...
export async function withScopedStatusToast<T>(
  ...
  try {
    ...
    if (messages.error)
      showToast.error(typeof messages.error === "function" ? messages.error(0) : messages.error, {
        entityId: id,
      });
    return undefined;
  }
...
export async function withToastQueue<T>(
  ...
  for (let i = 0; i < actions.length; i++) {
    const result = await withToast(actions[i], {
      ...messages,
      success: typeof messages.success === "function" ? messages.success(i) : messages.success,
      error: typeof messages.error === "function" ? messages.error(i) : messages.error,
    });
    results.push(result);
  }
  return results;
  ...
  return Promise.all(
    actions.map(async (action, i) => {
      try {
        const result = await action();
        showToast.success(
          typeof messages.success === "function" ? messages.success(i) : messages.success,
        ...
        const fallback = error instanceof Error ? error.message : String(error);
        showToast.error(
          typeof messages.error === "function" ? messages.error(i) : messages.error || fallback,
          { entityId: messages.entityId, duration: messages.duration }
        );
        return undefined;
      }
