...
export function resetTheme(options: ResetThemeOptions = {}) {
  ...
  if (options.scope && options.id && options.element) {
    const current = store.getPreferences(options.scope, options.id)?.preferences ?? {};
    updateScopedTheme(options.scope, options.id, current, defaultTheme, options.element);
  } else {
