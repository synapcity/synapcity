...
export function getDefaultSerializedState(minimal = false, config?: EditorConfig): string {
  const editor = createEditor(config);
  let obj: any;
 ...
 */
export function testRoundTripSerializer(editor: LexicalEditor, minimal = false): boolean {
...
export function getFlattenedLexicalData(serialized: string, config?: EditorConfig) {
  try {
...
export function serializeEditorState(editor: LexicalEditor, minimal = false): string {
...
export function getSafeInitialContent(content: string | undefined | null) {
  try {
    ...
    if (!parsed?.root || parsed.root.type !== "root") throw new Error("Invalid root");
    if (!Array.isArray(parsed.root.children) || parsed.root.children.length === 0)
