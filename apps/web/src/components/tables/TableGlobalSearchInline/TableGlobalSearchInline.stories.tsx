...
const makeStubTable = (rows: DataRow[], onFilter: (val: string) => void): Table<unknown> =>
  ({
    getCoreRowModel: () => ({
      rows: rows.map(
        (r) =>
          ({
            id: r.id,
            getAllCells: () => [{ getValue: () => r.firstName }, { getValue: () => r.lastName }],
          }) as any
      ),
    }),
...
export const Default: Story = {
  render: () => (
    <div style={{ padding: 16, maxWidth: 300 }}>
      <TableGlobalSearchInline table={makeStubTable(rawRows, action("filterChanged"))} />
    </div>
  ),
};
...
export const EmptyResults: Story = {
  render: () => (
    <div style={{ padding: 16, maxWidth: 300 }}>
      <TableGlobalSearchInline table={makeStubTable([], action("filterChanged"))} />
    </div>
  ),
};
